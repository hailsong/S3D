...
class UNetStyleDistil(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, init_features=64, bottleneck_features=512, repeat=1):
        super(UNetStyleDistil, self).__init__()

        # 512 x 512
        features = init_features
        self.encoder1 = self._block(in_channels, features, repeat=repeat)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)

        # 256 * 256
        self.encoder2 = self._block(features, features * 2, repeat=repeat)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)

        # 128 * 128
        self.encoder3 = self._block(features * 2, features * 4, repeat=repeat)
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)

        # 64 * 64
        self.encoder4 = self._block(features * 4, features * 8, repeat=repeat)
        self.pool4 = nn.MaxPool2d(kernel_size=2, stride=2)

        # 32 * 32
        self.encoder5 = self._block(features * 8, features * 8, repeat=repeat)
        self.pool5 = nn.MaxPool2d(kernel_size=2, stride=2)

        # 16 * 16
        self.encoder6 = self._block(features * 8, features * 8, repeat=repeat)
        self.pool6 = nn.MaxPool2d(kernel_size=2, stride=2)

        # 8 * 8
        self.encoder7 = self._block(features * 8, features * 8, repeat=repeat)
        self.pool7 = nn.MaxPool2d(kernel_size=2, stride=2)

        # Bottleneck (4 * 4)
        self.bottleneck = self._block(features * 8, bottleneck_features)
        ...
        self.decoder7 = self._block((features * 8) * 2, features * 8, repeat=repeat)

        # 16 * 16
        self.upconv6 = nn.ConvTranspose2d(features * 8, features * 8, kernel_size=2, stride=2)
        self.decoder6 = self._block((features * 8) * 2, features * 8, repeat=repeat)

        # 32 * 32
        self.upconv5 = nn.ConvTranspose2d(features * 8, features * 8, kernel_size=2, stride=2)
        self.decoder5 = self._block((features * 8) * 2, features * 8, repeat=repeat)

        # 64 * 64
        self.upconv4 = nn.ConvTranspose2d(bottleneck_features, features * 8, kernel_size=2, stride=2)
        self.decoder4 = self._block((features * 8) * 2, features * 8, repeat=repeat)

        # 128 * 128
        self.upconv3 = nn.ConvTranspose2d(features * 8, features * 4, kernel_size=2, stride=2)
        self.decoder3 = self._block((features * 4) * 2, features * 4, repeat=repeat)

        # 256 * 256
        self.upconv2 = nn.ConvTranspose2d(features * 4, features * 2, kernel_size=2, stride=2)
        self.decoder2 = self._block((features * 2) * 2, features * 2, repeat=repeat)

        # 512 * 512
        self.upconv1 = nn.ConvTranspose2d(features * 2, features, kernel_size=2, stride=2)
        self.decoder1 = self._block(features * 2, features, repeat=repeat)
    ...
    @staticmethod
    def _block(in_channels, features, repeat=1):
        layers = [
...
class UNetStyleDistilBackup(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, init_features=64, bottleneck_features=512):
        super(UNetStyleDistil, self).__init__()
